...
constexpr char APP_VERSION[] = "v0.7.2";
...
constexpr Rect WIFI_SHORTCUT_RECT{158, 444, 32, 32};
constexpr Rect ARCHIVE_SHORTCUT_RECT{198, 444, 32, 32};
...
constexpr Rect CALIBRATION_ENABLED_RECT{10, 308, 145, 44};
constexpr Rect CALIBRATION_RESET_RECT{165, 308, 145, 44};
...
{
    // Ikona patri iba do spodnej listy hlavnej obrazovky.
    // Ochrana zabrani jej prekresleniu cez tlacidla Wi-Fi menu.
...
void updateMessageStrip()
{
    ...
    char compactMessage[25];

    snprintf(
        compactMessage,
        sizeof(compactMessage),
        "%s",
        message
    );

    if (strlen(message) >= sizeof(compactMessage))
    {
        compactMessage[
            sizeof(compactMessage) - 4
        ] = '.';

        compactMessage[
            sizeof(compactMessage) - 3
        ] = '.';

        compactMessage[
            sizeof(compactMessage) - 2
        ] = '.';

        compactMessage[
            sizeof(compactMessage) - 1
        ] = '\0';
    }
...
void drawCalibrationEditor()
{
    ...
    tft.setTextDatum(MC_DATUM);
    tft.setTextColor(COLOR_TEXT, COLOR_PANEL);
    tft.drawString(title, 160, 21, 1);

    drawPanel(CALIBRATION_EDITOR_DISPLAY_RECT);
...
)
{
    ...
    if (CALIBRATION_RESET_RECT.contains(x, y))
    {
