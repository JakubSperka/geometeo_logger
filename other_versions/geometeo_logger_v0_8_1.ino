...
constexpr char APP_VERSION[] = "v0.8.1";
...
// Cierny zaklad zostava rovnaky; akcent je volitelny v nastaveniach.
...
uint16_t COLOR_PANEL_BORDER = 0x5980;
uint16_t COLOR_PRIMARY = 0xFC00;
uint16_t COLOR_ACCENT = 0xFD46;
...
enum class AccentTheme : uint8_t
{
    Orange = 0,
    Blue,
    Green,
...
struct AccentPalette
{
    ...
    const char *name;
    const char *webPrimary;
    const char *webBorder;
};
...
constexpr size_t ACCENT_THEME_COUNT =
    sizeof(ACCENT_PALETTES) /
    sizeof(ACCENT_PALETTES[0]);
...
constexpr Rect SETTINGS_THEME_RECT{242, 5, 68, 34};
...
// ============================================================
// ROZMERY VYBERU FAREBNEJ SCHEMY
// ============================================================
...
constexpr Rect THEME_SAVE_RECT{165, 414, 145, 50};
...
AccentTheme pendingAccentTheme =
    AccentTheme::Orange;
...
size_t accentThemeIndex(
    AccentTheme theme
)
{
    const size_t index =
        static_cast<size_t>(theme);

    return index < ACCENT_THEME_COUNT
        ? index
        : 0;
}
...
)
{
    return ACCENT_PALETTES[
        accentThemeIndex(theme)
    ];
}

void applyAccentTheme(
    AccentTheme theme
)
{
    ...
    // Varovny akcent sleduje zvolenu farebnu schemu.
    COLOR_WARNING = COLOR_PRIMARY;
}
...
void loadPersistentSettings()
{
    ...
    const uint8_t storedAccentTheme =
    ...
    if (storedAccentTheme < ACCENT_THEME_COUNT)
    {
        accentTheme =
    ...
    pendingAccentTheme = accentTheme;
...
void saveAccentThemePersistentSettings()
{
    preferences.begin("meteo", false);
...
)
{
    ...
    wifiExportServer.sendContent(
        F(
            ...
            ".info,.session{background:#1b1b1b;border:1px solid "
        )
    );
...
void drawSettingsScreen()
{
    ...
    tft.drawString(APP_VERSION, 8, 22, 1);
...
void drawThemeSettingsScreen()
{
    ...
    )
    {
        ...
        {
            tft.drawRoundRect(
                THEME_OPTION_RECTS[index].x - 2,
                THEME_OPTION_RECTS[index].y - 2,
                THEME_OPTION_RECTS[index].width + 4,
                THEME_OPTION_RECTS[index].height + 4,
                9,
                TFT_WHITE
            );
        }
...
void openThemeSettings()
{
    ...
    drawThemeSettingsScreen();
}

void cancelThemeSettings()
{
    pendingAccentTheme = accentTheme;
    applyAccentTheme(accentTheme);
    drawSettingsScreen();
...
void saveThemeSettingsAndReturn()
{
    accentTheme = pendingAccentTheme;
    applyAccentTheme(accentTheme);
    saveAccentThemePersistentSettings();

    setStatusMessage(
        "FAREBNA SCHEMA ULOZENA",
        COLOR_OK
    );

    Serial.printf(
        "FAREBNA SCHEMA: %s\n",
        accentPalette(accentTheme).name
    );

    drawSettingsScreen();
}

void processThemeSettingsTouch(
    int16_t x,
    int16_t y
)
{
    if (THEME_BACK_RECT.contains(x, y))
    {
        cancelThemeSettings();
        return;
    }

    if (THEME_SAVE_RECT.contains(x, y))
    {
        saveThemeSettingsAndReturn();
        return;
    }
    ...
    {
        if (
            THEME_OPTION_RECTS[index].contains(
                x,
                y
            )
        )
...
)
{
    ...
    if (SETTINGS_THEME_RECT.contains(x, y))
    {
        openThemeSettings();
        return;
    }
